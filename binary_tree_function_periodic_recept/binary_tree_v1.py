...
def remove_evt_from_active_sensors(elt_in_memory_that_will_die, type, simul_time, tau, children, parent):
    if len(children)==0 and len(parent)==0:
        return [],[]
    """print("REMOOVING")
    print(children)
    print(parent)
    ...
    if type == "children":
        ...
        if children_elt["next_emission"] <elt_in_memory_that_will_die["next_emission"]:
            children_elt["later_emission"] = elt_in_memory_that_will_die["next_emission"]
        else:
            children_elt["later_emission"] = elt_in_memory_that_will_die["next_emission"] + tau*math.pow(2,(len(elt_in_memory_that_will_die["id_tree"])))
        complementary_children["id_tree"] = complementary_children["id_tree"][:(len(complementary_children["id_tree"]) - 1)]
...
def get_elt_by_name(evt, children, parent):
    for i in range(len(children)):
        if children[i]["name"] == evt.name:
            children_elt = children.pop(i)
...
def add_new_sensor(evt, simul_time, tau, children,parent):
    ...
    parent, elt_that_will_be_a_children = get_elt_in_list_by_id_tree(elt_that_will_be_a_children["id_tree"], parent)
    height = len(elt_that_will_be_a_children["id_tree"])
    if "later_emission" in elt_that_will_be_a_children.keys():
        new_period = elt_that_will_be_a_children["later_emission"] - simul_time + tau*math.pow(2,(height))
    else:
        new_period = elt_that_will_be_a_children["next_emission"] - simul_time + tau*math.pow(2,(height))
    new_elt = {"name": evt.name, "next_emission": simul_time + new_period, "id_tree": elt_that_will_be_a_children["id_tree"]+"1"}
    elt_that_will_be_a_children["id_tree"] = elt_that_will_be_a_children["id_tree"] + "0"
    #elt_that_will_be_a_children["later_emission"] = elt_that_will_be_a_children["next_emission"] + tau*math.pow(2, (heignt+1))
    ...
    if evt is None:
        ...
        if children ==[] and parent == []:
            ...
            if not sensor_view_list.is_in(evt):
                ...
                children, parent,elt_in_memory, type = get_elt_by_name(evt, children, parent)
